...
+ (BOOL)isGzippedData:(NSData *)input {
    const UInt8 *bytes = (const UInt8 *)input.bytes;
    return (input.length >= 2 && bytes[0] == 0x1f && bytes[1] == 0x8b);
}

+ (NSData *)GZipData:(NSData *)input {
    if (input.length == 0 || [self isGzippedData:input]) {
        return input;
    }
    
    void *libz = [self libzOpen];
